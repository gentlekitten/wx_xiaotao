<template>
  <div>
    <!-- 顶部 navbar -->
    <top-search :tabIndex="tabIndex" @searchConfirm="searchConfirm" backTo="/index" />
    ...
    <tabs
      ...
      :sticky="true"
      offset-top="2.6rem"
      tabsIndexName="tabActiveIndexBeautyMakeup"
      @clickTab="clickTab"
    >
      <van-list
        v-model="loading"
...
export default {
  ...
  data() {
    return {
      ...
      finished: false,
      tabIndex: Number(
        window.sessionStorage.getItem('tabActiveIndexBeautyMakeup')
          ? window.sessionStorage.getItem('tabActiveIndexBeautyMakeup')
          : 0
      ),
      tabsList: [
